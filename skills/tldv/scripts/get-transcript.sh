#!/bin/bash
# Get tldv meeting transcript by ID
...
if [[ -z "$1" ]]; then
  echo '{"error": "Usage: get-transcript.sh <meeting_id>"}' >&2
  exit 1
fi

curl -s "$BASE_URL/meetings/$1/transcript" -H "x-api-key: $API_KEY" -H "Content-Type: application/json"
